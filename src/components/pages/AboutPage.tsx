...
import { Award, Palette, GraduationCap, Sparkles, CheckSquare, Target, Activity, User, Coffee, Utensils, Trophy, Star, Cpu, Mic } from 'lucide-react';
...
export default function AboutPage({ template }: PageProps) {
  ...
  const bioText = "I ask 'what if?' far too often. Somehow, it keeps turning into products and awards.";
  
  const hobbies = [
    { icon: Utensils, title: "Cooking", desc: "Constantly experimenting with healthier recipes and proving that \"healthy\" doesn't have to mean \"sad.\"" },
    { icon: Coffee, title: "Exploring Cafes & Restaurants", desc: "I enjoy discovering new places through Tiktok and try it once the hype died!" },
    { icon: Mic, title: "Singing", desc: "I don't have a favorite genre. If it sounds good, it's immediately my personality for the next three days." }
  ];

  const awards = [
    { icon: Trophy, title: "12+ Awards Won", desc: "Recognized excellence across national and international events." },
    { icon: Cpu, title: "4× Hackathon Winner", desc: "Local & International design and code hack sprints." },
    { icon: GraduationCap, title: "Summa Cum Laude", desc: "Top-tier academic excellence in Computer Science." },
    { icon: Star, title: "DOST Scholar & University Scholar", desc: "Prestigious national science and university scholarship programs." }
  ];

  const motto = "“Live your life beautifully. You only have one chance.”";
  const obsessionTitle = "Current Obsessions";
  const obsessionDesc = "Calorie tracking with Gemini • Practicing speaking skills through TalkGPT";
      ...
      <div className="max-w-3xl mx-auto pb-2 font-sans text-gray-900 space-y-12">
        ...
        <div className="flex flex-col md:flex-row gap-8 items-start">
          <div className="w-32 h-32 md:w-40 md:h-40 shrink-0 bg-gray-100 border border-gray-200 overflow-hidden select-none relative">
            {images.minimalist ? (
              <img 
                src="/about-minimalist.jpg" 
                alt="Jamie Ferrer Minimalist Portrait" 
                onError={() => setImages(prev => ({ ...prev, minimalist: false }))}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
          ...
          <div className="space-y-6">
            <div>
              <h2 className="text-xl font-bold tracking-tight text-gray-900 mt-1">Hobbies</h2>
            </div>
              ...
              {hobbies.map((h, i) => {
                return (
                  <div key={i} className="py-4 first:pt-0 last:pb-0 flex gap-4">
                    ...
                    <div>
                      <h3 className="font-bold text-sm text-gray-800">{h.title}</h3>
          ...
          <div className="space-y-6">
            <div>
              <h2 className="text-xl font-bold tracking-tight text-gray-900 mt-1">Awards & Achievements</h2>
            </div>
            <div className="divide-y divide-gray-150">
              {awards.map((a, i) => {
                return (
                  <div key={i} className="py-3.5 first:pt-0 last:pb-0 flex gap-4">
        ...
        <div className="space-y-2 py-2">
          <div className="flex gap-3 items-center">
            ...
            <p className="text-xs text-gray-700 leading-relaxed font-semibold">
              <span className="text-gray-900 font-bold">{obsessionTitle}:</span> {obsessionDesc}
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="max-w-4xl mx-auto pb-2 font-sans text-gray-900 space-y-12">
            ...
            <div className="w-full aspect-square bg-gray-100 overflow-hidden relative border-2 border-gray-950 rounded-none">
              {images.editorial ? (
                <img 
                  ...
                  onError={() => setImages(prev => ({ ...prev, editorial: false }))}
        ...
        <div className="w-full border-4 border-gray-950 bg-gray-950 p-6 rounded-none text-left flex flex-col justify-between">
          <p className="text-2xl md:text-4xl font-sans font-black uppercase tracking-tighter text-white leading-none">
            “PLATING VISUAL VECTORS WITH PREMIUM LAYOUTS”
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 pt-4">
          ...
          <div className="bg-white border-4 border-gray-950 p-6 rounded-none relative">
            <div className="mb-6 border-b-2 border-gray-900 pb-4">
              <h2 className="text-3xl font-sans font-black uppercase tracking-tighter text-gray-950 leading-none">Hobbies</h2>
            </div>
            <div className="space-y-6">
              {hobbies.map((h, i) => (
                <div key={i} className="flex gap-4 items-start p-4 hover:bg-gray-50 border-2 border-gray-950 bg-white rounded-none transition-colors">
                  ...
                  <div className="min-w-0">
                    <h3 className="font-black text-xs uppercase tracking-wide text-gray-950">{h.title}</h3>
                    <p className="text-xs text-gray-655 mt-1 leading-relaxed">{h.desc}</p>
                  </div>
                </div>
              ))}
            </div>
          ...
          <div className="bg-white border-4 border-gray-950 p-6 rounded-none relative">
            <div className="mb-6 border-b-2 border-gray-900 pb-4">
              <h2 className="text-3xl font-sans font-black uppercase tracking-tighter text-gray-950 leading-none">Accolades</h2>
            </div>
            <div className="space-y-4">
              {awards.map((a, i) => (
                <div key={i} className="flex gap-4 items-start p-4 hover:bg-gray-50 border-2 border-gray-950 bg-white rounded-none transition-colors">
                  ...
                  <div className="flex-1 min-w-0">
                    <h3 className="font-black text-xs uppercase tracking-wide text-gray-950 leading-tight">{a.title}</h3>
                    <p className="text-xs text-gray-655 mt-1 leading-normal">{a.desc}</p>
                  </div>
                </div>
              ))}
            </div>
        ...
        <div className="bg-white border-4 border-gray-950 p-5 rounded-none flex items-center justify-between gap-4">
          <div className="flex items-center gap-4">
            ...
            <div>
              <p className="text-[10px] font-mono uppercase font-black tracking-widest text-gray-950">Current Obsession</p>
              <p className="text-xs text-gray-700 font-bold uppercase mt-1">{obsessionTitle}: {obsessionDesc}</p>
            </div>
          </div>
        </div>
      </div>
    ...
    <div className="max-w-4xl mx-auto pb-2 font-mono text-[#00FF88] space-y-12">
      ...
      <div className="grid grid-cols-1 md:grid-cols-12 gap-8 items-center pt-2">
        ...
        <div className="col-span-1 md:col-span-4 flex justify-center">
          <div className="relative group p-1.5 border border-[#00FF88]/40 rounded-xl bg-black/40 shadow-[0_0_20px_rgba(0,255,136,0.15)]">
            ...
            <span className="absolute -top-1 -left-1 text-xs text-[#00FF88] font-black font-mono">┌</span>
            <span className="absolute -top-1 -right-1 text-xs text-[#00FF88] font-black font-mono">┐</span>
            <span className="absolute -bottom-1 -left-1 text-xs text-[#00FF88] font-black font-mono">└</span>
            <span className="absolute -bottom-1 -right-1 text-xs text-[#00FF88] font-black font-mono">┘</span>
            ...
            <div className="w-32 h-32 md:w-40 md:h-40 bg-black flex flex-col items-center justify-center border border-[#00FF88]/30 rounded-lg relative overflow-hidden">
              {images.futuristic ? (
                <img 
                  ...
                  onError={() => setImages(prev => ({ ...prev, futuristic: false }))}
                  className="w-full h-full object-cover filter brightness-90 saturate-125 border border-[#00FF88]/20"
                  referrerPolicy="no-referrer"
                />
              ) : (
                <Cpu size={36} className="text-[#00FF88] animate-pulse filter drop-shadow-[0_0_8px_var(--page-accent)]" />
              )}
            </div>
          </div>
        </div>
        ...
        <div className="p-6 rounded-xl border border-[#00FF88]/20 bg-[#11111b]/80 shadow-[0_0_15px_rgba(0,255,136,0.05)] text-white">
          ...
          <div className="space-y-4">
            {hobbies.map((h, i) => {
              ...
              return (
                <div key={i} className="flex gap-4 items-start p-2.5 border border-[#00FF88]/10 rounded-lg bg-black/30 hover:bg-black/55 transition-colors">
                  <HobbyIcon size={16} className="text-[#00FF88] mt-1 shrink-0 filter drop-shadow-[0_0_4px_var(--page-accent)]" />
                  <div className="space-y-0.5">
                    <h3 className="font-bold text-xs text-[#00FF88] uppercase">{h.title}</h3>
                    <p className="text-[10px] text-gray-400 mt-1 leading-normal">{h.desc}</p>
                  </div>
                </div>
              );
            })}
        ...
        <div className="p-6 rounded-xl border border-[#00FF88]/20 bg-[#11111b]/80 shadow-[0_0_15px_rgba(0,255,136,0.05)] text-white">
          <div className="flex items-center justify-between mb-4 border-b border-[#00FF88]/20 pb-2.5">
            <h2 className="text-sm font-black uppercase tracking-widest flex items-center gap-2 text-white">
              <Award size={14} className="text-[#00FF88]" /> Awards & Achievements
            </h2>
          </div>
          <div className="space-y-3">
            {awards.map((a, i) => {
              ...
              return (
                <div key={i} className="flex gap-3 items-start p-2 border border-[#00FF88]/10 rounded-lg bg-black/30 hover:bg-black/50 transition-colors">
                  <AwardIcon size={16} className="text-[#00FF88] mt-1 shrink-0 filter drop-shadow-[0_0_4px_var(--page-accent)]" />
                  <div className="space-y-0.5">
                    <h3 className="font-bold text-xs text-white leading-tight uppercase">{a.title}</h3>
                    <p className="text-[10px] text-[#00FF88]/80 mt-1">{a.desc}</p>
                  </div>
                </div>
              );
            })}
      ...
      <div className="p-5 rounded-xl border border-[#00FF88]/20 bg-black/30 flex items-center justify-between gap-4">
        <div className="flex items-center gap-4">
          ...
          <div>
            <p className="text-[8px] font-bold uppercase tracking-widest text-[#00FF88]/50">CURRENT OBSESSION</p>
            <p className="text-xs text-gray-300 font-bold tracking-tight uppercase mt-0.5">{obsessionTitle} :: {obsessionDesc}</p>
          </div>
        </div>
      </div>
    </div>
